...
namespace rtmiddleware {
	...
	*/
	...
	void RTCEditorItem::update_comp(const char* filename)
	{
...

		{
			...
			try
			{
				...
				if (pythonPlugin)
				{
					pythonPlugin->globalNamespace()["updateEditComp"](comp_name.c_str(), filename);
				}
	...
	void RTCEditorItem::activate_comp()
	{
...

		{
			...
			try
			{
				...
				if (pythonPlugin)
				{
					pythonPlugin->globalNamespace()["activateComp"](comp_name.c_str(), 0);
				}
	...
	void RTCEditorItem::deactivate_comp()
	{
...

		{
			...
			try
			{
				...
				if (pythonPlugin)
				{
					pythonPlugin->globalNamespace()["deactivateComp"](comp_name.c_str(), 0);
				}
	...
	void RTCEditorItem::reset_comp()
	{
...

		{
			...
			try
			{
				...
				if (pythonPlugin)
				{
					pythonPlugin->globalNamespace()["resetComp"](comp_name.c_str(), 0);
				}
			}
			catch (const py::error_already_set& e)
			{
	...
	void RTCEditorItem::add_dataport(RTC_XML::DataPorts port)
	{
...

		{
			...
			try
			{
				...
				if (pythonPlugin)
				{
					pythonPlugin->globalNamespace()["addDataPort"](comp_name.c_str(), port.get_name().toStdString().c_str(), port.get_portType().toStdString().c_str(), port.get_type().toStdString().c_str());
				}
			}
			catch (const py::error_already_set& e)
			{
...

		{
			...
			try
			{
				...
				if (pythonPlugin)
				{
					...
					comp_name = pybind11::str(pythonPlugin->globalNamespace()["createEditComp"](name)).cast<std::string>();
					/*
	...
	void RTCEditorItem::get_status(RTC_XML::RTC_ProfileRTP::RTC_State& status)
	{
...

		{
			...
			try
			{
				...
				if (pythonPlugin)
				{
					std::string status_str = pybind11::str(pythonPlugin->globalNamespace()["getStatus"](comp_name.c_str(), 0)).cast<std::string>();
					if (status_str == "ACTIVATE")
					{
						status = RTC_XML::RTC_ProfileRTP::RTC_State::RTP_Acitve;
					}
					else if (status_str == "INACTIVATE")
					{
						status = RTC_XML::RTC_ProfileRTP::RTC_State::RTP_InActive;
					}
					else if (status_str == "ERROR")
					{
						status = RTC_XML::RTC_ProfileRTP::RTC_State::RTP_Error;
					}
					else
					{
						status = RTC_XML::RTC_ProfileRTP::RTC_State::RTP_Created;
					}
				}
			}
			catch (const py::error_already_set& e)
	 ...
	 * @param ext
	 */
	bool RTCEditorItem::initialize(cnoid::ControllerIO* io)
	{
		return true;
	}
